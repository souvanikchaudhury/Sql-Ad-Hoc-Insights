...
WITH top_5_customers AS
  (SELECT c.customer_code,
          ...
          rank() over(ORDER BY avg(d.pre_invoice_discount_pct) DESC) AS rankk
   FROM dim_customer c
...
with req9 as
		(select channel,
		round(sum(gross_price*b.sold_quantity)/1000000,2) as Gross_Sales_min
...
group by channel 
order by Gross_Sales_min)
select *,
(Gross_sales_min*100)/sum(Gross_sales_min) over() as Percentage
from req9;
